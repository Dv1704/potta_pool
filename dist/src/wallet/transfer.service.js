...
let TransferService = class TransferService {
    prisma;
    transfer2FAService;
    MAX_TRANSFER_AMOUNT = 10000; // GHS
    DAILY_TRANSFER_LIMIT = 50000; // GHS per day
    MAX_TRANSFERS_PER_DAY = 20;
    // In-memory cache for daily limits (in production, use Redis)
    dailyLimits = new Map();
    constructor(prisma, transfer2FAService) {
        this.prisma = prisma;
        this.transfer2FAService = transfer2FAService;
        // Clean old daily limits every hour
        setInterval(() => this.cleanOldLimits(), 60 * 60 * 1000);
    }
    async initiateTransfer(senderId, recipientIdentifier, amount) {
        // Security validations
        if (amount <= 0)
            throw new BadRequestException('Amount must be positive');
        if (amount < 1)
            throw new BadRequestException('Minimum transfer amount is 1 GHS');
        ...
        if (requires2FA) {
            ...
            const { sessionId } = await this.transfer2FAService.generateAndSendCode(senderId, amount, recipientIdentifier);
            return {
                requires2FA: true,
                sessionId,
    ...
    async confirmTransfer(senderId, sessionId, code) {
        // Verify 2FA code
    ...
    async executeTransfer(senderId, recipientIdentifier, amount) {
