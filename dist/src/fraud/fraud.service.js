...
let FraudService = FraudService_1 = class FraudService {
    prisma;
    logger = new Logger(FraudService_1.name);
    constructor(prisma) {
        this.prisma = prisma;
    }
    async trackUserConnection(userId, ip) {
        if (!ip || ip.includes('127.0.0.1') || ip === '::1')
            return; // Skip local loopback in dev if needed, or track it
        try {
            const user = await this.prisma.user.findUnique({ where: { id: userId } });
            if (!user)
                return;
            const updateData = { lastLoginIp: ip };
    ...
    async checkMatchmakingFraud(playerIds, clientIps) {
    ...
    async checkGamePayoutFraud(gameId) {
        try {
            const game = await this.prisma.game.findUnique({
                where: { id: gameId },
                include: { influencer: true }
            });
            if (!game || game.status !== 'COMPLETED' || Number(game.stake) <= 0)
                return;
...
FraudService = FraudService_1 = __decorate([
    Injectable(),
