...
async function runE2E() {
    ...
    if (Number(p1Balance.available) !== 150)
        throw new Error(`P1 Balance Incorrect: ${p1Balance.available}`);
    ...
    if (Number(p1Lock.locked) !== 50 || Number(p1Lock.available) !== 100)
        throw new Error(`P1 Lock Incorrect: ${JSON.stringify(p1Lock)}`);
