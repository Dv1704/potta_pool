...
// -------------------------------------------------------------
let mockPocketedBalls: number[] = [];

const originalExecuteShot = PoolEngine.prototype.executeShot;
PoolEngine.prototype.executeShot = function (angle: number, power: number, sideSpin: number, backSpin: number) {
...
async function bootstrap() {
    ...
    // Wait for connection
    await Promise.all([
        new Promise<void>(resolve => socket1.on('connect', () => { console.log('   [Socket] Player 1 connected'); resolve(); })),
        new Promise<void>(resolve => socket2.on('connect', () => { console.log('   [Socket] Player 2 connected'); resolve(); }))
    ]);
    ...
    const gameEndedPromise = Promise.all([
        new Promise<void>((resolve, reject) => {
            ...
            socket1.on('gameEnded', (data: any) => {
                clearTimeout(timeout);
                console.log(`   🏁 Player 1 received gameEnded. Winner: ${data.winnerId}`);
                if (data.winnerId !== passiveUserId) reject(new Error('WinnerId mismatch in gameEnded'));
                resolve();
            });
        }),
        new Promise<void>((resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error('gameEnded timeout Player 2')), 20000);
            socket2.on('gameEnded', (data: any) => {
                clearTimeout(timeout);
    ...
    await Promise.all([
        new Promise<void>(resolve => socket3.on('connect', resolve)),
        new Promise<void>(resolve => socket4.on('connect', resolve))
    ]);

    const matchFoundPromise2 = Promise.all([
        new Promise<void>(resolve => socket3.on('matchFound', (data: any) => { gameId2 = data.gameId; resolve(); })),
        new Promise<void>(resolve => socket4.on('matchFound', () => resolve()))
    ]);
    ...
    const startMatchPromise2 = Promise.all([
        new Promise<void>(resolve => socket3.on('startMatch', () => resolve())),
        new Promise<void>(resolve => socket4.on('startMatch', () => resolve()))
    ]);
    ...
    const redisKey2 = (gameService as any).getGameKey(gameId2);
    const redisVal = await (gameService as any).redis.get(redisKey2);
    ...
    await (gameService as any).redis.set(redisKey2, JSON.stringify(stateObj));
    ...
    const expectedWinnerId2 = stateObj.players.find((id: string) => id !== activePlayerId2);
