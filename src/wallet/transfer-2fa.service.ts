...
@Injectable()
export class Transfer2FAService {
    private readonly LARGE_TRANSFER_THRESHOLD = 1000; // GHS
    private readonly CODE_EXPIRY_MINUTES = 10;
    private transferCodes = new Map<string, { code: string; amount: number; recipient: string; expiresAt: Date }>();
    ...
    async requiresConfirmation(amount: number): Promise<boolean> {
        return amount >= this.LARGE_TRANSFER_THRESHOLD;
    }

    async generateAndSendCode(userId: string, amount: number, recipientIdentifier: string): Promise<{ sessionId: string }> {
    ...
    async verifyCode(sessionId: string, code: string): Promise<{ amount: number; recipient: string }> {
