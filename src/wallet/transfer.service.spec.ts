...
describe('TransferService Security Tests', () => {
    ...
    describe('Input Validation', () => {
        it('should reject negative amounts', async () => {
            await expect(
                service.initiateTransfer('sender-id', 'recipient@example.com', -100)
            ).rejects.toThrow(BadRequestException);
        });

        it('should reject zero amounts', async () => {
            await expect(
                service.initiateTransfer('sender-id', 'recipient@example.com', 0)
            ).rejects.toThrow(BadRequestException);
        });

        it('should reject amounts below minimum (1 GHS)', async () => {
            await expect(
                service.initiateTransfer('sender-id', 'recipient@example.com', 0.5)
            ).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('Authorization Checks', () => {
        it('should prevent self-transfer', async () => {
            ...
            await expect(
                service.initiateTransfer('same-user-id', 'user@example.com', 100)
        ...
        it('should reject transfer to non-existent user', async () => {
            ...
            await expect(
                service.initiateTransfer('sender-id', 'nonexistent@example.com', 100)
