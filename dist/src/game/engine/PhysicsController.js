...
export class PhysicsController {
    ...
    _aPoints = [];
    _aPointsNormals = [];
    _aPointsNormalsTopLeft = [];
    _aPointsNormalsTopRight = [];
    _aPointsNormalsBottomLeft = [];
    _aPointsNormalsBottomRight = [];
    _aHoles = [];
    _aHolesTopLeft = [];
    _aHolesTopRight = [];
    _aHolesBottomLeft = [];
    ...
    _initCollisions() {
        ...
        for (let k = 0; k < Constants.FIELD_POINTS.length - 1; k++) {
            const oEdge = new Edge(Constants.FIELD_POINTS[k].x, Constants.FIELD_POINTS[k].y, Constants.FIELD_POINTS[k + 1].x, Constants.FIELD_POINTS[k + 1].y, k);
            this._aFieldEdges.push(oEdge);
        }
        const oLastEdge = new Edge(Constants.FIELD_POINTS[Constants.FIELD_POINTS.length - 1].x, Constants.FIELD_POINTS[Constants.FIELD_POINTS.length - 1].y, Constants.FIELD_POINTS[0].x, Constants.FIELD_POINTS[0].y, Constants.FIELD_POINTS.length - 1);
        ...
        oFirstNormal.set((this._aFieldEdges[0].getNormal().getX() + this._aFieldEdges[23].getNormal().getX()) / 2, (this._aFieldEdges[0].getNormal().getY() + this._aFieldEdges[23].getNormal().getY()) / 2);
        oFirstNormal.normalize();
        this._aPointsNormals.push(oFirstNormal);
        for (let i = 0; i < 23; i++) {
            const oTmpNormal = new Vector2((this._aFieldEdges[i].getNormal().getX() + this._aFieldEdges[i + 1].getNormal().getX()) / 2, (this._aFieldEdges[i].getNormal().getY() + this._aFieldEdges[i + 1].getNormal().getY()) / 2);
    ...
    verifyCollisionBallWithRectArea(vPos) {
    ...
    collideBallWithHoles(oBall, aHoles) {
    ...
    collideBallWithPointsNormals(oBall, vPos, aPointsNormals) {
