...
describe('Payment & FX (E2E)', () => {
    ...
    const mockFXService = {
        convertToGHS: jest.fn().mockImplementation(async (amount: any, currency: any) => {
            if (currency === 'GHS') return { ghsAmount: amount, rate: 1 };
            if (currency === 'USD') return { ghsAmount: amount * 16, rate: 16 };
    ...
    it('Admin Withdrawal: Should initiate transfer successfully', async () => {
        ...
        jest.spyOn(configService, 'get').mockImplementation((key: string) => {
            if (key === 'PAYSTACK_ADMIN_RECIPIENT_CODE') return 'RCP_12345';
