...
let EmailService = class EmailService {
    ...
    getFromEmail() {
        return this.configService.get('RESEND_FROM_EMAIL') || 'Potta <noreply@playpotta.com>';
    }
    async postEmail(to, subject, html, from) {
        if (!this.apiKey) {
    ...
    wrapInTemplate(title, bodyContent) {
        return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body style="margin: 0; padding: 0; background-color: #0b0f19; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; min-height: 100%;">
    ...
    async sendPasswordResetEmail(to, token) {
        try {
            const html = this.wrapInTemplate('Reset Your Potta Password', `
    ...
    async sendWelcomeEmail(to, name) {
        try {
            const html = this.wrapInTemplate('Welcome to Potta!', `
    ...
    async sendVerificationCodeEmail(to, name, code) {
        try {
            const html = this.wrapInTemplate('Verify Your Email Address', `
    ...
    async sendTransfer2faEmail(to, name, amount, recipientIdentifier, code) {
        try {
            const html = this.wrapInTemplate('Transfer Confirmation Code', `
    ...
    async sendWithdrawal2faEmail(to, name, amount, mobileNumber, code) {
        try {
            const html = this.wrapInTemplate('Withdrawal Confirmation Code', `
