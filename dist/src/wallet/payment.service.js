...
let PaymentService = PaymentService_1 = class PaymentService {
    ...
    async verifyTransaction(reference, userId) {
        try {
            ...
            const amount = data.amount / 100; // Convert minor units (kobo/pesewas) back
     ...
     */
    async handlePaystackWebhook(payload, rawBody, signature) {
    ...
    async initiateUserWithdrawal(userId, amount, mobileNetwork, mobileNumber) {
        ...
        await this.emailService.sendWithdrawal2faEmail(user.email, user.name || 'Potta User', amount, mobileNumber, code);
        return {
            requires2FA: true,
            sessionId,
     ...
     */
    async confirmUserWithdrawal(userId, sessionId, code) {
...
PaymentService = PaymentService_1 = __decorate([
    ...
    __param(3, Inject(FXService)),
    __param(4, Inject(forwardRef(() => AdminService))),
    __param(5, Inject(EmailService)),
