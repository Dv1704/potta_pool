...
async function testAviatorTimeout() {
    ...
    try {
        ...
        const betResponse = await axios.post(`${API_URL}/game/aviator/bet`, { stake: 10 }, { headers: { Authorization: `Bearer ${authToken}` } });
...
async function testDiceErrorHandling() {
    ...
    try {
        ...
        const response = await axios.post(`${API_URL}/game/play/dice`, { stake: 5 }, { headers: { Authorization: `Bearer ${authToken}` } });
        ...
        try {
            await axios.post(`${API_URL}/game/play/dice`, { stake: -5 }, { headers: { Authorization: `Bearer ${authToken}` } });
...
async function testCoinGame() {
    ...
    log('─'.repeat(60), colors.blue);
    try {
        const response = await axios.post(`${API_URL}/game/play/coin`, { stake: 3, choice: 'heads' }, { headers: { Authorization: `Bearer ${authToken}` } });
...
async function runAllTests() {
    ...
    try {
        await axios.post(`${API_URL}/payments/deposit/initialize`, { amount: 1000, currency: 'GHS' }, { headers: { Authorization: `Bearer ${authToken}` } });
